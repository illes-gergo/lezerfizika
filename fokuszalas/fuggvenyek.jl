...
function superPos(startI, endI, nu, spektrum, zGrid, xGrid::LazyGrids.GridSL)
    supPos = zeros(Complex{Float64}, size(zGrid)..., nthreads())
    @threads for ind in startI:endI
        supPos[:, :, threadid()] += gaussNyalab(xGrid, zGrid, t, spektrum[ind], 3e8 / nu[ind])
    end
    return dropdims(sum(supPos, dims=3),dims=3)
end


function superPos(startI, endI, nu, spektrum, zGrid, xGrid::Number)
