function diffegy(t, current_values)
    ...
    q = current_values[2]
    na = current_values[3]
    tauc = max(eta * L^3 / 8 / c / pi^2 * n * V^2 * (sigmaell - sigmalll), eta * L / 10 / c)

    dndt = I(t) * sigmap * (N - n) - sigmaell * c / eta * n * q - n / tau
    dqdt = (sigmaell - sigmalll) * c / eta * n * q - q / tauc + omega * n / tau - sigmaaal * c / eta * na * q
    dnadt = -I(t) * sigmapa * na + sigmaeal * c / eta * (Na - na) * q + (Na - na) / taua - sigmaaal * c / eta * na * q + 2 * omegaa / taua / L / (sigmaeaa - sigmalaa) * (exp((sigmaeaa - sigmalaa) * (Na - na)*L) - 1)

    return [dndt, dqdt, dnadt]
end

function tauc_(n)
    return max(eta * L^3 / 8 / c / pi^2 * n * V^2 * (sigmaell - sigmalll), eta * L / 10 / c)
end
